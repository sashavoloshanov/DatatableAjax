...
class SimpleDataTableWidget {
    ...
    createPerson(firstName, lastName, age, city) {
        ...
        $.post(this.dbUrl + '/person', newUser)
            .done((data, status) => {
    ...
    renderWidget() {
        ...
        for (let i = firstDisplayI; i < lastDisplayI; i++) {
            let rowHtml = $('<tr>');
            rowHtml.append($('<td>').html('<input type="checkbox" name="type" value="' + this.displayPersons[i]._id + '" />'));
            ...
            rowHtml.append($('<td id="key' + i + '" hidden>').text(this.displayPersons[i]._id));
    ...
    downloadData(done) {
        $.get(this.dbUrl + '/person', (data, status) => {
            console.log({data: data, status: status});
...
function onClickUpdatePerson(event) {
    ...
    datatableonj.updateInputKey.val(datatableonj.displayPersons[index]._id);
}
